$btnGuardar.onclick = async () => {
    ...
    try {
        ...
        if (respuesta) {
            ...
            Swal.fire({
                icon: "success",
                text: "persona guardada",
                timer: 700, // <- Ocultar dentro de 0.7 segundos
            });
            // Limpiamos el formulario
            $nombre.value = $apellido.value = $apellidoMaterno.value = $rut.value = $correo.value = $profesion.value = $direccion.value =  $region.value = "";
        } else {
            Swal.fire({
                icon: "error",
